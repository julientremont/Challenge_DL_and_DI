...
class GitHubReposSilverProcessor:
    ...
    def clean_and_normalize(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Clean repository names - remove special characters, normalize
        ...
        # Ensure numeric fields are properly typed and handle negatives
        ...
        for field in numeric_fields:
            ...
            df.loc[:, field] = df[field].clip(lower=0)  # No negative counts
        
        # Clean and validate created_at timestamps - ensure timezone naive
        ...
        # Clean collected_at timestamps - ensure timezone naive  
    ...
    def _recalculate_metrics(self, df: pd.DataFrame) -> pd.DataFrame:
        """Recalculate activity metrics with cleaned data"""
        # Calculate days since creation - ensure both timestamps are timezone naive
        ...
        # Age penalty for very old repos without recent activity
    ...
    def _calculate_quality_score(self, df: pd.DataFrame) -> pd.Series:
        ...
        # Deduct for very old repos (might be stale)
        quality_score -= (df['days_since_creation'] > 3650) * 10  # 10 years
        
        # Deduct for repos with no activity
        quality_score -= (df['popularity_score'] == 0) * 20
        
        # Deduct for suspicious names
